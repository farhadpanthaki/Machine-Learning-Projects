...
pop_size = 32 #should be divisible by 2
...
##    X_current = X.loc[:,population[i] == 1] #use population as on/off switch along each col to select features for accuracy evaluation
    fitness.append( rsbrk_fitness(population[i],a,b,lower_l,upper_l))#fitness is a pandas series containing the accuracy values for the different chromosomes in the population
...
##    #Tournament Selection
    ...
    for i in mutated_population.columns:
        fitness_children.append(rsbrk_fitness(mutated_population.iloc[::,i],a,b,lower_l,upper_l))
    ...
    new_fitness = pd.concat([sorted_fitness.iloc[tournament_result[0:parents_retained.astype(int)]],sorted_fitness_children.iloc[0:(pop_size - parents_retained.astype(int))]],axis = 0,ignore_index = True) #Top 3 of tournament winners and top pop_size - 3 from the sorted mutated population
    ...
    fitness = new_fitness.sort_values()
